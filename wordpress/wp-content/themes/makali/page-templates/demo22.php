...
if(count($jscomposer_templates) > 0) {
	foreach($jscomposer_templates as $jscomposer_template){
		if($jscomposer_template->post_title == 'Header 8' || $jscomposer_template->post_title == 'Footer 4'){
...
<body <?php body_class('home'); ?>>
	...
	<div class="wrapper">
		<div class="page-wrapper">
			<div class="header-container header-8 demo-22 rs-active">  
			 	<div class="header">
					<div class="header-content">
						...
						if ( isset($makali_opt['header_layout']) && $makali_opt['header_layout']!="") {
							...
							if(count($jscomposer_templates) > 0) {
								foreach($jscomposer_templates as $jscomposer_template){
									if($jscomposer_template->post_title == 'Header 8'){
										?>
							...
							if ( isset($makali_opt['header_mobile_layout']) && $makali_opt['header_mobile_layout'] != "") {
								if(count($jscomposer_templates) > 0) {
									foreach($jscomposer_templates as $jscomposer_template){
										if($jscomposer_template->post_title == 'Header Mobile 2'){ ?>
											<div class="header-mobile header-mobile-2">
												<div class="container">
													<?php 
														echo do_shortcode(apply_filters( 'the_content', $jscomposer_template->post_content ));
													?>
			...
			<div class="footer footer-4">
				<div class="footer-inner">
					...
					if ( isset($makali_opt['footer_layout']) && $makali_opt['footer_layout']!="" ) {
						...
						if(count($jscomposer_templates) > 0) {
							foreach($jscomposer_templates as $jscomposer_template){
								if($jscomposer_template->post_title == 'Footer 4'){
									echo do_shortcode(apply_filters( 'the_content', $jscomposer_template->post_content ));
								}
							}
						}
