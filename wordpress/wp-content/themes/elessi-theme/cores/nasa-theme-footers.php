...
if (!function_exists('elessi_static_config_info')) :
    function elessi_static_config_info() {
        ...
        <?php
        echo (defined('NASA_PLG_CACHE_ACTIVE') && NASA_PLG_CACHE_ACTIVE) ? '<input type="hidden" name="nasa-caching-enable" value="1" />' : '';
    }
endif;
...
if (!function_exists('elessi_link_style_off_canvas')) :
    function elessi_link_style_off_canvas() {
        echo '<input name="nasa-style-off-canvas" id="nasa-style-off-canvas" type="hidden" value="' . esc_url(ELESSI_THEME_URI . '/assets/css/style-off-canvas.css') . '" />';
    }
endif;

/**
 * Header Menu Mobile template
 */
add_action('nasa_after_static_content', 'elessi_template_menu_mobile');
if (!function_exists('elessi_template_menu_mobile')) :
    function elessi_template_menu_mobile() {
        global $nasa_opt;
        if (isset($nasa_opt['nasa_in_mobile']) && $nasa_opt['nasa_in_mobile']) {
            ?>
            <script type="text/template" id="tmpl-nasa-menu-mobile">
                <?php
                $file = ELESSI_CHILD_PATH . '/headers/menu-mobile.php';
                include is_file($file) ? $file : ELESSI_THEME_PATH . '/headers/menu-mobile.php';
                ?>
            </script>
            <?php
        }
