...
$('body').on('nasa_before_init_menu_mobile', function() {
    if ($('#tmpl-nasa-menu-mobile').length) {
        var _tmpl_content = $('#tmpl-nasa-menu-mobile').html();
        $('#tmpl-nasa-menu-mobile').replaceWith(_tmpl_content);
    }
});
 ...
 * Close MagnificPopup
 */
$('body').on('click', '.nasa-mfp-close', function() {
    $.magnificPopup.close();
});
...
if (typeof search_options !== 'undefined' && typeof search_options.url !== 'undefined') {
    $('body').on('focus', '.live-search-input', function() {
        ...
        if (!$(_this).hasClass('nasa-inited')) {
            $('.live-search-input').addClass('nasa-inited');
...
$('body').on('click', '.nasa_add_to_cart_from_wishlist', function() {
    ...
    if (_id && !$(_this).hasClass('loading')) {
        ...
        // nasa_single_add_to_cart($, _this, _id, _quantity, _type, null, null, _data_wishlist);
    }
...
$('body').on('nasa_append_style_off_canvas', function() {
    if ($('head').find('#elessi-style-off-canvas-css').length <= 0 && $('#nasa-style-off-canvas').length) {
        var _link_style = $('#nasa-style-off-canvas').val();
        $('head').append('<link rel="stylesheet" id="elessi-style-off-canvas-css" href="' + _link_style + '" />');
    }
});
